<template>
    <div>
      ...
      <form @submit.prevent="handleSaveScore" class="bg-gray-800 p-4 rounded-lg shadow-lg mb-6 max-w-md overflow-hidden">
      ...
      <div class="bg-gray-800 p-4 rounded-lg shadow-lg">
        ...
        <ul v-else class="space-y-2"> <li v-for="(entry, index) in scores" :key="entry.id"
              class="flex flex-wrap justify-between items-center bg-gray-900 px-3 py-2 rounded shadow-sm gap-2"> <div class="flex items-center space-x-3 mr-4">
              <span class="font-bold text-lg text-center w-6 shrink-0" :class="getRankPillColor(index)">{{ index + 1 }}</span>
              <span class="font-semibold text-gray-100 truncate">{{ entry.group_name }}</span>
            </div>
            <div class="flex items-center space-x-2"> <BaseButton size="sm" variant="secondary" @click="changeScore(entry, -1)" :disabled="entry.score === 0 || loadingStates[entry.id]"> - </BaseButton>
              <span class="text-lg font-bold w-8 text-center">{{ entry.score }}</span>
              <BaseButton size="sm" variant="secondary" @click="changeScore(entry, 1)" :disabled="loadingStates[entry.id]"> + </BaseButton>
              <BaseButton size="sm" variant="danger" @click="handleDelete(entry.id)" :disabled="loadingStates[entry.id]" class="ml-2" aria-label="Verwijder"> <span class="i-mdi-delete w-4 h-4"></span>
              </BaseButton>
            </div>
            <div v-if="loadingStates[entry.id]" class="w-full text-center text-xs text-gray-400 pt-1">Bezig...</div>
          </li>
        </ul>
      </div>
    </div>
