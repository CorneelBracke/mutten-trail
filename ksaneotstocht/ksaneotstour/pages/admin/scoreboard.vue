<template>
    <div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="group_name" v-model="form.group_name" required
                        class="block w-90% px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-gray-200 focus:outline-none focus:ring-1 focus:ring-brand-yellow focus:border-brand-yellow sm:text-sm" />
                </div>
                <div>
        ...
        <div class="w-80% bg-gray-800 p-4 rounded-lg shadow-lg">
            <h3 class="text-xl font-semibold mb-4 text-gray-300">Huidige Stand</h3>
            <ul v-if="scores && scores.length > 0" class="space-y-2 max-w-2xl">
                <li v-for="(entry, index) in scores" :key="entry.id"
                    class="flex flex-wrap justify-between items-center bg-gray-800 px-4 py-2 rounded shadow gap-2">
                    <div class="flex items-center space-x-3 mr-4">
                        <span class="font-bold text-lg text-brand-yellow w-6 text-center shrink-0">{{ index + 1
                            }}.</span>
                    ...
                    <div class="flex items-centerspace-x-2">
                        ...
                        <BaseButton size="sm" variant="danger" @click="handleDelete(entry.id)"
                            :disabled="loadingStates[entry.id]" class="ml-4">
...
// ... (definePageMeta, useHead, form state zoals voorheen) ...
definePageMeta({ layout: 'admin', middleware: 'auth' });
useHead({ title: 'Admin - Scoreboard' });
const form = reactive({ group_name: '', score: 0 as number | null });
const loadingSubmit = ref(false);
const formError = ref<string | null>(null);
const loadingStates = reactive<Record<number, boolean>>({}); // Loading state per entry
...
// Formulier submit (voegt toe of zet initiele score)
async function handleSaveScore() { /* ... (zoals voorheen) ... */ loadingSubmit.value = true; formError.value = null; if (typeof form.score !== 'number') { formError.value = 'Score moet een getal zijn.'; loadingSubmit.value = false; return; } try { await $fetch('/api/admin/scoreboard', { method: 'POST', body: { group_name: form.group_name, score: form.score } }); form.group_name = ''; form.score = 0; await refresh(); } catch (error: any) { console.error("Error saving score:", error); formError.value = error.data?.message || 'Kon score niet opslaan.'; } finally { loadingSubmit.value = false; } }

// Score aanpassen (+/- knoppen)
async function changeScore(entry: ScoreboardEntry, change: number) {
    if (loadingStates[entry.id]) return; // Voorkom dubbelklikken
    loadingStates[entry.id] = true; // Start loading state voor deze rij

    const newScore = (entry.score ?? 0) + change;
    if (newScore < 0) { // Optioneel: voorkom negatieve scores
        console.warn("Score can't be negative");
        loadingStates[entry.id] = false;
        return;
    }


    console.log(`Changing score for ID ${entry.id} to ${newScore}`);
    try {
        await $fetch(`/api/admin/scoreboard/${entry.id}`, {
            method: 'PATCH',
            body: { score: newScore } // Stuur de nieuwe absolute score
        });
        await refresh(); // Herlaad de hele lijst
    } catch (error: any) {
        console.error(`Error changing score for ${entry.id}:`, error);
        alert(`Kon score niet aanpassen: ${error.data?.message || 'Serverfout'}`);
    } finally {
        loadingStates[entry.id] = false; // Stop loading state
    }
}
...
async function handleDelete(id: number) {
    ...
    if (!confirm(`Weet je zeker dat je score entry #${id} wilt verwijderen?`)) return;

    loadingStates[id] = true; // Gebruik ID ook hier voor loading state
    console.log('Deleting score entry:', id);
    try {
        await $fetch(`/api/admin/scoreboard/${id}`, { method: 'DELETE' });
        await refresh();
    } catch (error: any) {
        console.error("Delete failed:", error);
        alert(`Kon score entry niet verwijderen: ${error.data?.message || 'Serverfout'}`);
    } finally {
        // Verwijder key, anders blijft hij bestaan als de rij weg is
